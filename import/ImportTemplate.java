        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(file.getInputStream()));
            ...
            while ((line = csvReader.readNext()) != null) {
                if (isHeader) {
                    ...
                    // Si la ligne a plus de colonnes que l'en-tête
                    if (line.length > headerLength) {
                        String[] adjustedLine = new String[headerLength];
                        // Copier les valeurs jusqu'à l'avant-dernière colonne de l'en-tête
                        System.arraycopy(line, 0, adjustedLine, 0, headerLength - 1);
                        
                        // Concaténer l'avant-dernière et la dernière valeur avec un point
                        String lastValue = line[headerLength - 1];
                        String extraValue = line[line.length - 1];
                        adjustedLine[headerLength - 1] = lastValue + "." + extraValue;
                        
                        // Ajouter les valeurs restantes si nécessaire (mais normalement non car adjustedLine a la taille du header)
                        line = adjustedLine;
                    }
